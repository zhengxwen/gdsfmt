...
n2 <- index.gdsn(gfile, "I2")
apply.gdsn(n2, 1, FUN=cat, as.is="none", file=fout, fill=TRUE)
close(fout)
...
```{r}
fout <- file("text.txt", "wt")
apply.gdsn(n, 1, FUN=cat, as.is="none", file=fout, fill=32768)
close(fout)
...
```{r}
gfile <- openfn.gds("test.gds", readonly=FALSE)
n <- index.gdsn(gfile, "I1")
